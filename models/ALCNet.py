    ...
    def __init__(self,
        ...
        self.block = nn.Sequential(
            nn.Conv2d(in_channels=in_channels,
                      ...
                      kernel_size=3,
                      padding=1,
                      bias=False), norm_layer(inter_channels,
                                              momentum=momentum),
...
class ASKCResNetFPN(nn.Module):
    ...
    def __init__(self,
        ...
        if tinyFlag:
            ...
            self.stem = nn.Sequential(
                ...
                nn.ReLU(inplace=True),
                nn.Conv2d(in_channels=stem_width,
                          out_channels=stem_width,
                          kernel_size=3,
                          stride=1,
                          padding=1,
            ...
            self.deconv2 = nn.ConvTranspose2d(
                ...
                stride=2,
                padding=1)
            self.uplayer2 = self._make_layer(block=BasicBlock,
                                             blocks=layers[1],
                                             out_channels=channels[2],
                                             stride=1,
                                             in_channels=channels[2])

            self.deconv1 = nn.ConvTranspose2d(in_channels=channels[2],
                                              out_channels=channels[1],
                                              kernel_size=(4, 4),
                                              stride=2,
                                              padding=1)
            ...
            self.deconv0 = nn.ConvTranspose2d(
                in_channels=channels[1],
                out_channels=2 * channels[0],
                kernel_size=(4, 4),  ###########out_channels###########
                stride=2,
                padding=1)

            self.uplayer1 = self._make_layer(block=BasicBlock,
                                             blocks=layers[0],
                                             out_channels=channels[1],
                                             stride=1,
                                             in_channels=channels[1])
            ...
            if self.layer_num == 4:
                self.fuse34 = self._fuse_layer(
                    fuse_mode, channels=channels[3])  # channels[4]

            self.fuse23 = self._fuse_layer(fuse_mode,
                                           channels=channels[2])  # 64
            self.fuse12 = self._fuse_layer(fuse_mode,
    ...
    def forward(self, x):
        ...
        if self.tinyFlag:
            out = transforms.Resize([hei // 2, wid // 2])(
                out)  # down 16 torch.Size([8, 64, 64, 64])
        else:
...
class ALCNet_DTUM(nn.Module):
    ...
    def __init__(self,
        ...
        self.ALCNet = ASKCResNetFPN(layers,
                                    channels,
                                    fuse_mode,
                                    act_dilation,
                                    classes=32,

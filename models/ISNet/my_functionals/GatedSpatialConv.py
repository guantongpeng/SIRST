...
import models.ISNet.my_functionals.custom_functional as myF
...
class HighFrequencyGatedSpatialConv2d(_ConvNd):
    ...
    def forward(self, input_features, gating_features):
        ...
        dog_features = self.cw(torch.cat((dog_features, input_features),
                                         dim=1))
...
def t():
    ...
    canny_map = np.random.normal(size=(1, canny_map_filters_in, 10,
                                       10))  # NxCxHxW
    ...
    gconv = GatedSpatialConv2d(canny_map_filters_in,

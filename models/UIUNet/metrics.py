...
class SigmoidMetric():
    ...
    def batch_intersection_union(self, output, target):
        ...
        maxi = 1  # nclass
        nbins = 1  # nclass
        predict = (output.detach().numpy() > 0.22).astype(
            'int64')  # P   ###########阈值，不应该直接>0,会有很小的值 0.22  合成数据0.8
        target = target.numpy().astype('int64')  # T
        intersection = predict * (predict == target)  # TP

        # areas of intersection and union
        area_inter, _ = np.histogram(intersection,
                                     bins=nbins,
                                     range=(mini, maxi))
...
class SamplewiseSigmoidMetric():
    ...
    def batch_intersection_union(self, output, target, nclass, score_thresh):
        ...
        predict = (F.sigmoid(output).detach().numpy()
        ...
        for b in range(num_sample):
            # areas of intersection and union
            area_inter, _ = np.histogram(intersection[b],
                                         bins=nbins,
                                         range=(mini, maxi))
            area_inter_arr[b] = area_inter

            area_pred, _ = np.histogram(predict[b],
                                        bins=nbins,
                                        range=(mini, maxi))
            area_pred_arr[b] = area_pred

            area_lab, _ = np.histogram(target[b],
                                       bins=nbins,
                                       range=(mini, maxi))

...
class GlobalFilter(nn.Module):
    ...
    def forward(self, x):
        _, C, H, W  = x.shape
        x = torch.fft.rfft2(x, dim=(2, 3), norm='ortho')
        weight = torch.view_as_complex(torch.randn(C, H, int(W/2+1), 2, dtype=torch.float32, device='cuda') * 0.02)
        x = x * weight
        x = torch.fft.irfft2(x, s=(H, W), dim=(2, 3), norm='ortho')   
        return x

class TFRes_CBAM_block(nn.Module):
    def __init__(self, in_channels, out_channels, stride = 1):
        super(TFRes_CBAM_block, self).__init__()
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            global_out = x
        if self.shortcut is not None:
            global_out = self.fft_cut(global_out)
...
class TFDNANet(nn.Module):
    def __init__(self, num_classes=1,input_channels=1, block=TFRes_CBAM_block, num_blocks=[2, 2, 2, 2], nb_filter=[16, 32, 64, 128, 256], deep_supervision=True, mode='test'):
        super(TFDNANet, self).__init__()
        self.mode = mode
        self.relu = nn.ReLU(inplace = True)

...
class DTUM(nn.Module):  # final version
    ...
    def direction(self, arr):
        ...
        up_down = torch.Tensor(range(0, m)).cuda(
            arr.device) * n * 2 * 2  #.transpose(0,1)
    ...
    def forward(self, x):
        ...
        x_2 = self.relu(
            self.bn2_1(torch.abs(self.conv2_1(xp_1 * self.direction(ind)))))
        xp_2, ind = self.pool(x_2)
        x_3 = self.relu(
            self.bn3_1(torch.abs(self.conv3_1(xp_2 * self.direction(ind)))))
        xp_3, ind = self.pool(x_3)
        x_4 = self.relu(
            self.bn4_1(torch.abs(self.conv4_1(xp_3 * self.direction(ind)))))

        o_3 = self.relu(
            self.bn3_2(self.conv3_2(torch.cat([self.up(x_4), x_3], dim=1))))
        o_2 = self.relu(
            self.bn2_2(self.conv2_2(torch.cat([self.up(o_3), x_2],
                                              dim=1)))).detach()
        o_1 = self.relu(
            self.bn1_2(self.conv1_2(torch.cat([self.up(o_2), x_1], dim=1))))

        x_out = self.final(
            torch.cat([o_1, torch.unsqueeze(x[:, :, -1, :, :], 2)], dim=1))

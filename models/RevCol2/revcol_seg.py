...
from .modules import ConvNextBlock, Decoder, LayerNorm, SimDecoder, UpSampleConvnext
...
class FullNet(nn.Module):
    def __init__(self, channels=[32, 64, 96, 128], layers=[2, 3, 6, 3], num_subnet=5, kernel_size = 3, num_classes=1000, drop_path = 0.0, save_memory=True, inter_supv=True, head_init_scale=None) -> None:
        ...
        self.stem = nn.Sequential(
            nn.Conv2d(1, channels[0], kernel_size=4, stride=4),
        ...
        if not inter_supv:
            ...
            self.cls_blocks = nn.ModuleList([Decoder(depth=[1,1,1,1], dim=channels, block_type=ConvNextBlock, kernel_size = 3) for _ in range(3) ])
            if num_classes<=1000:
                channels.reverse()
                self.decoder_blocks = nn.ModuleList([Decoder(depth=[1,1,1,1], dim=channels, block_type=ConvNextBlock, kernel_size = 3) for _ in range(4) ])
            else:
